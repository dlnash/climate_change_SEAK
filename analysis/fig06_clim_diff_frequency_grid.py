"""
Filename:    clim_diff_frequency_grid.py
Author:      Deanna Nash (adapted for multi-model comparison)
Description: Plot 3-column x 5-row comparison of CFSR climatology and model differences for ROS frequency, rain, snow, and IVT > threshold frequency
"""
...
# Plot Function
# ============================================================
def plot_ros_diff_grid(models, varnames, path_to_data,
    ...
    fname = os.path.join(path_to_data, f"preprocessed/SEAK-WRF/cfsr/trends/cfsr_freq_rain_snow_ivt.nc")
    ...
    for i, varname in enumerate(varnames):
        ...
        if (varname == 'ros'):
            ...
            levs_clim, cmap_clim = np.arange(0, 110, 10), cmo.deep
            ...
            levs_clim, cmap_clim = np.arange(0, 330, 30), cmo.rain
            levs_diff = np.arange(-40, 48, 8)
            cmap_diff, norm = make_brgr_white_cmap(levs_diff, (-4, 4))
        ...
        for j, model in enumerate(models[1:], start=3):
            ...
            fname = os.path.join(path_to_data, f"preprocessed/SEAK-WRF/{model}/trends/{model}_freq_rain_snow_ivt.nc")
        ...
        if varname == 'ros':
            cb1.set_label(f"Δ ROS days yr$^{{-1}}$")
        else:
            cb1.set_label(f"Δ {cfsr_ds[varname].attrs.get('units', '')}")

    # ============================================================
    # Save Figure
    # ============================================================
    outname = f"../figs/MODEL_DIFF_FREQ_GRID.png"
...
if __name__ == "__main__":
    ...
    plot_ros_diff_grid(models, varnames, path_to_data)
