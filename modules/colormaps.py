...
def get_colormap_and_levels(clim_type, varname):
    ...
    if clim_type == "95th_percentile_clim":
        if varname == 'ivt':
            levs_clim = np.arange(150, 650, 50); cmap_clim = cmo.deep
            levs_diff = np.arange(20, 130, 10);  cmap_diff = cmo.deep
        elif varname == 'uv':
            levs_clim = np.arange(0, 55, 5);    cmap_clim = cmo.dense
            levs_diff = np.arange(-2.5, 3, .5);    cmap_diff = cmo.balance
            ...
            levs_clim = np.arange(12, 78, 6);    cmap_clim = cmo.rain
            levs_diff = np.arange(-10, 12, 2);  cmap_diff = 'BrBG'
        elif varname == 'snow':
            levs_clim = np.arange(400, 2600, 200); cmap_clim = cmo.rain
